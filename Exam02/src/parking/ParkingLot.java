...
//	  (4) 입력된 차량번호와 일치하는 정보가 없는 경우 "대상 차량이 존재하지 않습니다." 메시지를 출력한다.
	...
	public void deleteCar() {
		...
		for(int i = 0; i < idx; i++) {
			...
			if(carNo.equals(car.getCarNo())) {  
				...
				*/
				...
				// 방법2. 삭제할 요소의 뒤에 있는 것들을 모두 한 칸씩 앞으로 옮기는 방법
				System.arraycopy(cars, (i + 1), cars, i, (idx - i - 1));  // cars를 i+1부터의 자료를 cars의 i 자리에 (idx-i-1)만큼 이동
				cars[--idx] = null;
		...
		// 동일한 차량번호가 없을 경우(else로 구현하면 조회될 때마다 출력됨)
