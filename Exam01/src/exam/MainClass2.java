...
public class MainClass2 {
	...
	public static void q1() {
		/*
	 	풀이 방법은 다양함
	 	1. 자릿수에 맞는 단위(1000000)로 나눠서 몫으로 비교하는 경우
	 	2. 10으로 계속 나눠서 몫으로 비교하는 경우
	 	3. 아래와 같은 방법
		 */
	...
	public static void q2() {
		...
		int sub = a - b;
		int mul = a * b;
		double div = (double)a / b;  // 모두 정답 : a / (double)b , (double)a / (double)b
	...
	public static void q3() {
		...
		for(int dan = 2; dan <= 5; dan++) {
			for(int n = 1; n <= 9; n++) {
				...
				if(dan == 5 && n == 5) {
					break;  // return이 들어가도 가능
				}
			}
		}
		*/
		// 아래 내용은 별로 추천하는 스타일은 아님(label은 가급적 사용하지말기)
		outer:  // 바깥 for문 label
	...
	public static void q6() {
		...
		for(int i = 0; i < arr.length; i++) {
			if(arr[i] < 0) {
				continue;  // 0보다 작으면 total을 진행하지마라.
			}
			total += arr[i];
		}
		*/
